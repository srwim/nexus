...
import { DEFAULT_PREFS, normalizeLeagues } from "./topics";
...
export function loadPrefs() {
  ...
  try {
    ...
    return {
      ...DEFAULT_PREFS,
      ...saved,
      // "racing" retired when Motor Racing split into four series; anyone whose
      // only sport was racing would otherwise silently lose it.
      leagues: normalizeLeagues(saved.leagues),
...
function applySharedLink(prefs) {
  try {
    ...
    const next = {
      ...
      ...(Array.isArray(shared.leagues) ? { leagues: normalizeLeagues(shared.leagues) } : {}),
