...
// and inherits all the dedup work. Sports is prebuilt too now, one entry per
// league in sports.json, so the email carries the same paddock/beat coverage the
// site does instead of an ESPN-only fallback.
//
...
export async function buildPublishedDigest(prefs, siteUrl) {
  ...
  const entries = await Promise.all(
    activeTopics(prefs.ratings).map(async (key) =>
      key === "sports"
        ? [key, { items: fetchSportsFrom(await getJson(`${base}data/sports.json`), prefs.leagues, 20) }]
