...
import { decodeEntities, stripHtml, dedupeByTitle, decodeFeedBody } from "./text.js";

export { decodeEntities, filterObituaries, decodeFeedBody } from "./text.js";
...
async function fetchFeed(url) {
  ...
  try {
    ...
    // A non-2xx used to return [] in silence, so a feed that started 403ing
    // datacenter IPs looked exactly like a feed with no news, which is how a
    // whole topic can go dark unnoticed. Status codes are the cheapest signal
    // there is; log them.
    if (!res.ok) {
      console.warn(`  feed failed: ${url} (HTTP ${res.status})`);
      return [];
    }
