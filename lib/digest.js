...
import { fetchSportsLive } from "./sportsLive.js";
...
export async function buildDigest(prefs) {
  ...
  const raw = await Promise.all(
    active.map(async (topicKey) => {
      ...
      if (topicKey === "sports") {
        const items = await fetchSportsLive(prefs.leagues, over);
  ...
  const sections = raw.map((s) => {
    ...
    return { ...s, items: (s.items || []).filter((it) => keep(it)).slice(0, s.budget) };
  });
