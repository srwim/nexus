...
import { selectSponsors, mergeSponsors, describeSponsors, safeUrl, PLACEMENTS } from "./sponsors.js";
...
// ── link safety ──────────────────────────────────────────────────────────────
// A sponsor URL arrives by email from whoever bought the slot, and it ends up
// as a clickable link in a newsletter readers trust. It is attacker-adjacent
// input and gets treated as such.

test("only http and https are ever linkable", () => {
  assert.ok(safeUrl("https://acme.example/"), "https is fine");
  assert.ok(safeUrl("http://acme.example/"), "http is fine");
  for (const hostile of [
    ...
    assert.equal(safeUrl(hostile), null, `${JSON.stringify(hostile)} must never become a link`);
  }
});

test("an unsafe sponsor URL renders as text, never as an anchor", () => {
  const c = paid("acme", { url: "javascript:alert(1)" });
  const s = selectSponsors(data([house, c]), "2026-09-01");
  assert.equal(s.primary.url, "", "no href at all");
  assert.equal(s.primary.title, "acme headline", "the ad still shows, it just isn't clickable");
});
...
test("a malformed sponsor URL degrades instead of throwing", () => {
  ...
  assert.equal(selectSponsors(data([house, c]), "2026-09-01").primary.url, "");
});

// ── click tracking links ─────────────────────────────────────────────────────

test("tracking is off unless switched on", () => {
  const s = selectSponsors(data([house, paid("acme")]), "2026-09-01", "https://w.example");
  assert.match(s.primary.url, /^https:\/\/acme\.example\//, "trackClicks is unset, so links stay direct");
});

test("a tracking link carries the campaign id, never the destination", () => {
  const d = { ...data([house, paid("acme")]), trackClicks: true };
  const s = selectSponsors(d, "2026-09-01", "https://w.example");
  const u = new URL(s.primary.url);
  assert.equal(u.origin, "https://w.example");
  assert.equal(u.pathname, "/c");
  assert.equal(u.searchParams.get("id"), "acme");
  assert.equal(u.searchParams.get("p"), "primary");
  // The thing that would make arok.ai an open redirect.
  assert.equal(u.searchParams.get("u"), null);
  assert.ok(!/acme\.example/.test(s.primary.url), "the destination is not in the link at all");
});

test("an unsafe destination is not given a tracking link either", () => {
  const d = { ...data([house, paid("acme", { url: "javascript:alert(1)" })]), trackClicks: true };
  const s = selectSponsors(d, "2026-09-01", "https://w.example");
  assert.equal(s.primary.url, "", "a tracking link to nothing is worse than no link");
});

test("a hostile tracking base is rejected rather than trusted", () => {
  const d = { ...data([house, paid("acme")]), trackClicks: true };
  const s = selectSponsors(d, "2026-09-01", "javascript:alert(1)");
  assert.match(s.primary.url, /^https:\/\/acme\.example\//, "falls back to the direct link");
});
...
// ── resolveSponsors: the one answer to "which ads run today" ────────────────
// These exist because build-brief.js and send-newsletter.mjs used to answer it
// separately, and one of them answered wrong for weeks.
import { resolveSponsors } from "./sponsors.js";

const HOUSE_ONLY = {
  house: "sol-tek",
  campaigns: [
    { id: "sol-tek", house: true, placements: ["top", "primary", "footer"], title: "Sol-Tek", url: "https://sol-tek.us/" },
  ],
};
const SPONSY_EMPTY = async () => ({ top: null, primary: null, footer: null });

test("resolveSponsors fills every placement from the house ad", async () => {
  const out = await resolveSponsors(HOUSE_ONLY, "2026-09-18", {});
  for (const p of ["top", "primary", "footer"]) {
    assert.equal(out[p]?.title, "Sol-Tek", `${p} was left empty`);
    assert.equal(out[p].house, true);
  }
});

test("Sponsy is not consulted while enableSponsy is off", async () => {
  let called = false;
  const out = await resolveSponsors(HOUSE_ONLY, "2026-09-18", {
    fetchSponsy: async () => { called = true; return { top: null, primary: null, footer: null }; },
  });
  assert.equal(called, false, "a disabled integration should not be called");
  assert.equal(out.primary.title, "Sol-Tek");
});

// The regression. An empty Sponsy result must never displace the house ad.
test("an all-empty Sponsy result does not blank the placements", async () => {
  const out = await resolveSponsors({ ...HOUSE_ONLY, enableSponsy: true }, "2026-09-18", {
    fetchSponsy: SPONSY_EMPTY,
  });
  for (const p of ["top", "primary", "footer"]) {
    assert.equal(out[p]?.title, "Sol-Tek", `${p} was blanked by an empty Sponsy response`);
  }
});

test("a real Sponsy ad fills a placement the house ad was only borrowing", async () => {
  const out = await resolveSponsors({ ...HOUSE_ONLY, enableSponsy: true }, "2026-09-18", {
    fetchSponsy: async () => ({ top: null, primary: { title: "Paid Co", url: "https://paid.example" }, footer: null }),
  });
  assert.equal(out.primary.title, "Paid Co", "a paying sponsor outranks the house ad");
  assert.equal(out.top.title, "Sol-Tek", "placements Sponsy had nothing for keep the house ad");
});

test("no campaigns at all yields empty placements rather than throwing", async () => {
  const out = await resolveSponsors({ campaigns: [] }, "2026-09-18", {});
  for (const p of ["top", "primary", "footer"]) assert.equal(out[p], null);
});

test("missing sponsor data never throws", async () => {
  const out = await resolveSponsors(undefined, "2026-09-18", {});
