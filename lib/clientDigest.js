...
export async function assembleDigest(prefs) {
  ...
  const raw = await Promise.all(
    active.map(async (key) => {
      ...
      if (key === "sports") {
        ...
        if (data?.updatedAt) updatedAt = data.updatedAt;
        return { ...base, type: "news", items: fetchSportsFrom(data, prefs.leagues, over) };
      }
      if (key === "foreign") {
        const data = await getData("foreign");
        if (data?.updatedAt) updatedAt = data.updatedAt;
        return { ...base, type: "news", items: foreignFrom(data, prefs.countries, over) };
      }
  ...
  //
  // `diversify`, not `slice` — a plain slice hands the section to whoever posts
  // most often, which is how Gaming ended up eight-for-eight Polygon on the live
  // site even after the newsletter path had been fixed.
  const keep = makeDeduper();
  const sections = raw.map((s) => {
    if (s.type !== "news") return s;
    return { ...s, items: diversify((s.items || []).filter((it) => keep(it)), s.budget) };
  });
