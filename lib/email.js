...
export function renderEmailHtml(digest, extras = {}) {
  ...
  const teaserHtml = teaser.length
    ? `<div style="margin-top:14px;padding-top:12px;border-top:1px solid #23232b;color:#d1d5db;font-size:13px;line-height:1.6;">
        <span style="color:#6ee7b7;font-weight:800;letter-spacing:0.5px;">Digest</span>&nbsp; ${teaser
          .map((t) => `<a href="${esc(t.link)}" style="color:#e7e7ee;text-decoration:none;border-bottom:1px solid #3f3f46;">${esc(t.title)}</a>`)
          .join(" ")}
      </div>`
    : "";
