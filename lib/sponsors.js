...
// The single answer to "which ads run today", for every caller.
//
// It exists because there used to be two answers. When sponsors moved from
// Sponsy to this repo's sponsors.json, send-newsletter.mjs was taught the new
// path and build-brief.js was not: it kept calling the Sponsy fetcher, which
// with Sponsy switched off returns { top: null, primary: null, footer: null }.
// That object is truthy, so the send's `prebuilt?.sponsors ?? selectSponsors(…)`
// accepted it as a real answer and never reached the house ad. Every gated
// brief went out with three empty placements while sponsors.json sat there
// correctly configured.
//
// Sponsy fetching lives in scripts/, so it is injected rather than imported:
// this module stays dependency-free and browser-safe.
export async function resolveSponsors(data, today, { trackBase = "", fetchSponsy = null, debug = false } = {}) {
  const local = selectSponsors(data, today, trackBase);
  if (!data?.enableSponsy || !fetchSponsy) return local;
  return mergeSponsors(local, await fetchSponsy({ debug }), data, today);
}
