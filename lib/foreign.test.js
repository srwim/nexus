...
test("a publisher that writes dates in its own language is not treated as stale", () => {
  // Youm7 stamps items "الخميس، 20 أغسطس 2026 06:13 م". Date.parse returns NaN,
  // and the strict filter read that as stale and deleted all of Egypt.
  const arabic = [
    { title: "مجلس الوزراء يوافق على قرارات", link: "https://www.youm7.com/1", date: "الخميس، 20 أغسطس 2026 06:13 م" },
    { title: "البنك المركزى يثبت الفائدة", link: "https://www.youm7.com/2", date: "الخميس، 20 أغسطس 2026 05:28 م" },
  ];
  assert.equal(filterStale(arabic, 4, Date.now(), { keepUndated: true }).length, 2);
  assert.equal(filterStale(arabic, 4).length, 0, "the strict default is unchanged for US feeds");
});

test("keepUndated still drops items with a readable but genuinely old date", () => {
  const old = [{ title: "Ancient news", link: "https://x.test/1", date: "2020-01-01T00:00:00Z" }];
  assert.equal(filterStale(old, 4, Date.now(), { keepUndated: true }).length, 0);
});
