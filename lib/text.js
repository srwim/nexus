...
// Drop items older than `days`. Local news needs this because a city-scoped
// query asked for N results returns the N most recent EVER — so a town with
// little coverage backfills the list with two-week-old stories rather than
// returning fewer. Showing three fresh items beats ten stale ones.
// Undated items are dropped: unverifiable age fails closed in a news product.
// `keepUndated` decides what an unparseable date means.
//
// For US feeds, no readable date means something is wrong and dropping the item
// is right. For foreign-language feeds it means the publisher writes dates in
// its own language — Youm7 stamps items "الخميس، 20 أغسطس 2026", which
// Date.parse cannot read — and treating that as stale deletes the entire
// country. Undated items are kept but sort last, since we genuinely don't know
// when they ran.
export function filterStale(items, days = 4, now = Date.now(), { keepUndated = false } = {}) {
  const cutoff = now - days * 86400000;
  return (items || []).filter((it) => {
    const t = Date.parse(it?.date ?? "");
    if (!Number.isFinite(t)) return keepUndated;
    return t >= cutoff;
  });
}
...
// Unicode letters and numbers, not [a-z0-9]. Foreign Reporting carries Arabic,
// Japanese, Chinese and Korean headlines, and an ASCII-only key reduces every
// one of them to "" — which the de-duplicator reads as "already seen" and drops
// the entire section after the first story. It also keeps accents, so Spanish
// and Portuguese headlines stop colliding on their stripped forms.
...
function summaryKey(summary) {
  const t = String(summary || "").toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "");
...
// Decode the feed body using the encoding it was actually sent in.
//
// res.text() always assumes UTF-8. Plenty of publishers still ship ISO-8859-1:
// Folha de S.Paulo's rss091.xml does, and every accented byte in it came out as
// U+FFFD, so "Proposta que prevê código" reached the page as "Proposta que
// prev? c?digo". Worse than ugly, it is lossy: the replacement character is
// unrecoverable, so the translator was being handed text with holes in it.
//
// Strict UTF-8 first, because text that decodes cleanly as UTF-8 is essentially
// never accidentally valid; a single invalid byte throws and we fall back to the
// declared legacy encoding. windows-1252 is the default fallback rather than
// ISO-8859-1: it is a superset, so it also covers the smart quotes publishers
// mislabel as Latin-1.
export function decodeFeedBody(buffer, contentType = "") {
  ...
  try {
    ...
    // Not UTF-8. Believe the Content-Type header, then the XML declaration.
    ...
    try {
      ...
      // An encoding label Node doesn't know. windows-1252 maps every byte to
      // something, so this always produces a string rather than throwing.
