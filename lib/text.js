...
// A stateful de-duplicator. Call keep(title) for each headline in priority
// order; it returns true the first time a story is seen and false for later
// near-duplicates. Reuse ONE instance across multiple sections to stop the same
// story appearing in more than one (e.g. a TechCrunch AI story in both Tech and
// AI), or across a single list for ordinary de-duplication.
export function makeDeduper(threshold = 0.8) {
  const keys = new Set();
  const kept = []; // { words, dist } per surviving headline
  return function keep(title) {
...
export function dedupeByTitle(items, threshold = 0.8) {
  ...
  return items.filter((it) => keep(it.title));
}
